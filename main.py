...
# --- SOZLAMALAR ---
# Sizning haqiqiy tokeningiz qo'yildi
    ...
    for uid, data in user_data.items():
        if data.get('notes'):
            ...
            for i, n in enumerate(data['notes']):
                ...
                report += f"{i+1}. {n['text']} (saqlanganiga {diff} kun bo'ldi) ⏳\n"
            try:
                await bot.send_message(uid, report)
            except:
                pass

# --- WEB SERVER VA SCHEDULER (Render uyg'oq turishi uchun) ---
...
# --- KO'RISH FUNKSIYASI (Tugma bosilganda ishlashi uchun) ---
...
async def handle_msg(message: types.Message):
    # 1. Ko'rish tugmasi bosilganda
    if message.text == BTN_VIEW:
        await show_all_notes(message, message.from_user.id)
        return

    # 2. Instagram linki bo'lsa (darhol yuklaydi)
    ...
    # 3. Oddiy matn bo'lsa (tasdiqlash so'raydi)
...
# --- CALLBACK TUGMALARI (Ha/Yo'q/O'chirish) ---
...
async def main():
    # Conflict xatosini oldini olish uchun avvalgi webhookni o'chirish
